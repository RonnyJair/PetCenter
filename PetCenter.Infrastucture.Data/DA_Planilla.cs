...
namespace PetCenter.Infrastucture.Data
{
    public class DA_Planilla
    {
        ...
        public List<Planilla> findPlanillas(DateTime Fecha)
        {
            try
            {
                using(BdPetCenterEntities1 contexto = new BdPetCenterEntities1())
                {
                    List<Planilla> objeto = (from x in contexto.Planillas.Include("PlanillaEmpleadoes")
                                             where x.Fecha.Value.Year == Fecha.Year && x.Fecha.Value.Month == Fecha.Month && x.Fecha.Value.Day == Fecha.Day
                                             select x).ToList();
                    return objeto;
                }
        ...
        public List<PlanillaEmpleado> findPlanillaDetalle(Planilla planilla)
        {
            try
            {
                using(BdPetCenterEntities1 contexto = new BdPetCenterEntities1())
                {
                    return contexto.PlanillaEmpleadoes.Where(s => s.PlanillaId == planilla.PlanillaId).ToList();
                }
            }
            catch(System.Exception ex)
            {
